...
export function Button({children = "Button", type = 1, clickEvent}: props){
    return(
        <button onClick={clickEvent} className={`px-4 py-1 ${type == 1? "bg-secondary text-white hover:bg-white hover:text-secondary active:bg-secondary active:text-white" : "bg-white text-primary hover:bg-slate-600 hover:text-white"} rounded-md transition-all`}>{children}</button>
    )
}
